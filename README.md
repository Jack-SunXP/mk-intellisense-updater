...
- 安装或更新 `.vsix` 之后请执行一次 `Developer: Reload Window`（重新加载窗口）。
  右键菜单文字在窗口加载时解析，未重新加载时可能仍显示为英文；扩展的解析逻辑
  同样需要重新加载后才会更新。
...
- Discovers include-path variables generically: no fixed variable name is
  required. Any make variable whose value is a list of directories that exist on
  disk qualifies, and the names actually used are reported in the result message.
- Detects the build root per makefile, resolving relative entries against the
  nearest ancestor directory that makes them exist.
- Supports `=` / `:=` / `?=` / `+=` / `!=` assignments, backslash continuations,
  recursive `$(VAR)` expansion, and the `-I` / `-D` / `-L` / `-l` compiler flags.
- Detects linked libraries (`.a` and `.lib`) and optionally merges include
  settings from their library build files.
- Removes duplicate include paths while preserving order; only directories that
  exist on disk are written.
...
3. (Optional) Tick the linked libraries (`.a` / `.lib`) to resolve; all detected entries are pre-selected.
4. (Optional) Confirm which library `.mk` files to merge include paths from (the match score is shown).
5. Pick the IntelliSense configuration to update, or create a new one.
6. The extension writes the updated IntelliSense settings and reports the source
   variable(s), the detected build root, and how many entries were skipped.

### Input Requirements

- The selected build file (`.mk` file) should contain include-related configuration entries.
- If related library artifacts (such as `xxx.a` or `xxx.lib`) are referenced, corresponding library build settings can be merged during update.
- Documentation examples use placeholder names only (for example, `module_xxx.mk`, `lib_xxx.a`).

### Notes

- After installing or updating the `.vsix`, run `Developer: Reload Window`. Menu
  labels are resolved when the window loads, so without a reload the context menu
  can still appear in English, and the resolution logic is not refreshed either.
...
- Include paths are checked against the file system before being written. Entries
  that do not exist are skipped and reported as `N/M path entries ... were skipped`.
...
- `mkintellisense.directoryRatioThreshold`
  - The fraction of a variable's entries that must resolve to existing directories
    before the variable counts as an include-path source. Raise it to avoid picking
    up variables that merely look like path lists.
  - Default: `0.9` (range `0.5`–`1`)
- `mkintellisense.minTokensPerVariable`
  - The minimum number of entries a variable must have to be considered.
  - Default: `2` (range `1`–`10`)
- `mkintellisense.includeVariableSelection`
  - Which qualifying variables are used as include-path sources. Header directories
  are usually named with inc/include/hdr/header; source and link directories are not
  header roots, and adding them makes the C/C++ parser index the whole tree.
  - `inc-only` (default): only variables whose name contains inc/include/hdr/header.
  - `prefer`: prefer inc-named variables, fall back to all qualifying ones if none.
  - `all`: use every variable whose entries resolve to existing directories.
